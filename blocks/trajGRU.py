...
def wrap(input, flow):
    B, C, H, W = input.size()
    # mesh grid
    xx = torch.arange(0, W).view(1, -1).repeat(H, 1).to(args.device)
    yy = torch.arange(0, H).view(-1, 1).repeat(1, W).to(args.device)
...
class BaseConvRNN(nn.Module):
    def __init__(self, num_filter, b_h_w,
        ...
        self._height, self._width = b_h_w
...
if __name__ == '__main__':
    ...
    model = TrajGRU(input_channel=3, num_filter=8, b_h_w=( 256, 256), zoneout=0.0, L=13,
                    i2h_kernel=(3, 3), i2h_stride=(1, 1), i2h_pad=(1, 1),
                    h2h_kernel=(3, 3), h2h_dilate=(1, 1),
                    act_type=args.rnn_act).to(args.device)
