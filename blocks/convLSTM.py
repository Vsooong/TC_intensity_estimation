...
class ConvLSTM(nn.Module):
    def __init__(self, input_channel, num_filter, b_h_w, kernel_size, stride=1, padding=1):
        ...
        self._conv = nn.Conv2d(in_channels=input_channel + num_filter,
                               ...
                               stride=stride,
                               padding=padding)
        self._state_height, self._state_width = b_h_w
        ...
        self.Wci = nn.Parameter(torch.zeros(1, num_filter, self._state_height, self._state_width)).to(args.device)
        self.Wcf = nn.Parameter(torch.zeros(1, num_filter, self._state_height, self._state_width)).to(args.device)
        self.Wco = nn.Parameter(torch.zeros(1, num_filter, self._state_height, self._state_width)).to(args.device)
...
if __name__ == '__main__':
    ...
    input = torch.rand(12, 3, 3, 256, 256).to(args.device)
    model = ConvLSTM(input_channel=3, num_filter=8, b_h_w=(256, 256), kernel_size=3)
